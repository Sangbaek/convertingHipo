...
// MC bank
    T->Branch("MCEpx",&MCEpx,"MCEpx[nml]/F");
    T->Branch("MCEpy",&MCEpy,"MCEpy[nml]/F");
    T->Branch("MCEpz",&MCEpz,"MCEpz[nml]/F");
    T->Branch("MCEvx",&MCEvx,"MCEvx[nml]/F");
    T->Branch("MCEvy",&MCEvy,"MCEvy[nml]/F");
    T->Branch("MCEvz",&MCEvz,"MCEvz[nml]/F");
    T->Branch("MCPpx",&MCPpx,"MCPpx[nml]/F");
    T->Branch("MCPpy",&MCPpy,"MCPpy[nml]/F");
    T->Branch("MCPpz",&MCPpz,"MCPpz[nml]/F");
    T->Branch("nmG", &nmG, "nmG/I");
    T->Branch("MCGpx",&MCGpx,"MCGpx[nmG]/F");
    T->Branch("MCGpy",&MCGpy,"MCGpy[nmG]/F");
    T->Branch("MCGpz",&MCGpz,"MCGpz[nmG]/F");
...
// MC bank
	auto idx_MCPart = c12.addBank("MC::Particle");
   	...
   	auto iMCVx  = c12.getBankOrder(idx_MCPart,"vx");
   	auto iMCVy  = c12.getBankOrder(idx_MCPart,"vy");
	auto iMCVz  = c12.getBankOrder(idx_MCPart,"vz");
	       ...
	       Before[ipa] = val;
               ...
               Ftof2Sector[ipa] = tempFtof2Sector;
            	...
            	for(auto ipa=0;ipa<c12.getBank(idx_MCPart)->getRows();ipa++){
                	...
                	auto tMCPx = c12.getBank(idx_MCPart)->getFloat(iMCPx,ipa);
                	auto tMCPy = c12.getBank(idx_MCPart)->getFloat(iMCPy,ipa);
                	auto tMCPz = c12.getBank(idx_MCPart)->getFloat(iMCPz,ipa);
                	auto tMCVx = c12.getBank(idx_MCPart)->getFloat(iMCVx,ipa);
                	auto tMCVy = c12.getBank(idx_MCPart)->getFloat(iMCVy,ipa);
                	auto tMCVz = c12.getBank(idx_MCPart)->getFloat(iMCVz,ipa);
 
                	if( (c12.getBank(idx_MCPart)->getInt(iMCPid,ipa)) == 11  ){  // electrons
				MCEpx[nml] = tMCPx;
				MCEpy[nml] = tMCPy;
				MCEpz[nml] = tMCPz;
				MCEvx[nml] = tMCVx;
				...
				MCEvz[nml] = tMCVz;
	                ...
	                if((c12.getBank(idx_MCPart)->getInt(iMCPid,ipa)) == 2212  ){  // protons
                    		MCPpx[nml] = tMCPx;
                    		MCPpy[nml] = tMCPy;
                    		MCPpz[nml] = tMCPz;
                	} // if for protons
                    
                	if((c12.getBank(idx_MCPart)->getInt(iMCPid,ipa)) == 22  ){  // photons
                    		MCGpx[nmG] = tMCPx;
                    		MCGpy[nmG] = tMCPy;
                    		MCGpz[nmG] = tMCPz;
				nmG++;
