...
int main(int argc, char **argv){
    ...
    TFile *rFile = TFile::Open("MC.root","RECREATE");
    TTree *T=new TTree("T","MC");
...
/*
// main particle bank ========
	auto idx_RECPart = c12.addBank("MC::Particle");
...
// MC particle bank ========
	auto idx_MCPart = c12.addBank("MC::Particle");
   	auto iPid = c12.getBankOrder(idx_MCPart,"pid");
    	auto iPx  = c12.getBankOrder(idx_MCPart,"px");
    	auto iPy  = c12.getBankOrder(idx_MCPart,"py");
   	auto iPz  = c12.getBankOrder(idx_MCPart,"pz");
	auto iVx  = c12.getBankOrder(idx_MCPart,"vx");
	auto iVy  = c12.getBankOrder(idx_MCPart,"vy");
	auto iVz  = c12.getBankOrder(idx_MCPart,"vz");
	auto iVt  = c12.getBankOrder(idx_MCPart,"vt");
//	auto iCh  = c12.getBankOrder(idx_MCPart,"charge");
//	auto iB  = c12.getBankOrder(idx_MCPart,"beta");
//        auto iStat = c12.getBankOrder(idx_MCPart,"status");
//===================
/*
...
*/
            	...
            	for(auto ipa=0;ipa<c12.getBank(idx_MCPart)->getRows();ipa++){
                
                	auto tPx = c12.getBank(idx_MCPart)->getFloat(iPx,ipa);
                	auto tPy = c12.getBank(idx_MCPart)->getFloat(iPy,ipa);
                	auto tPz = c12.getBank(idx_MCPart)->getFloat(iPz,ipa);
			auto tVx = c12.getBank(idx_MCPart)->getFloat(iVx,ipa); 
			auto tVy = c12.getBank(idx_MCPart)->getFloat(iVy,ipa);
			auto tVz = c12.getBank(idx_MCPart)->getFloat(iVz,ipa);
			auto tVt = c12.getBank(idx_MCPart)->getFloat(iVt,ipa);
			//auto tB = c12.getBank(idx_MCPart)->getFloat(iB,ipa);
			//auto tStat = c12.getBank(idx_MCPart)->getInt(iStat,ipa);

 
                	if( (c12.getBank(idx_MCPart)->getInt(iPid,ipa)) == 11  ){  // electrons
                    		p4_electron[nml].SetXYZM(tPx,tPy,tPz,0.5109989461 * 0.001);
...
//				Ephi[nml]   = TMath::ATan2(Epy[nml],Epx[nml]);
				...
				EorigIndx[nml] = ipa;//Before[ipa];  // Original row index before skiming 
				//Esector[nml] = PcalSector[ipa];
				nml++;
                	}
                
	                if((c12.getBank(idx_MCPart)->getInt(iPid,ipa)) == 2212  ){  // protons
...
//		    		int status = tStat;
                                ...
                                PorigIndx[nmb] = ipa;//Before[ipa];  // Original row index before skiming 
...
/*			    	if(status<4000){
                	...
                	if((c12.getBank(idx_MCPart)->getInt(iPid,ipa)) == 22  ){  // photons
...
//                                Gstat[nmg] = tStat;
                                ...
                                GorigIndx[nmg] = ipa; //Before[ipa];  // Original row index before skiming 
